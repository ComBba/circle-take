...
- Cinematic cut, not a slideshow: real on-screen motion (Three.js hero rotating, the
  autonomous-roll scroll montage, the two real Wan clips), slow zoom-in on detail
  frames, a punch-zoom "CUT!" slam, crossfades on every cut.
...
- **Narration is generated by Qwen Cloud TTS** (`qwen3-tts-flash`, voice "Ethan") —
  the demo's voice is itself a Qwen Cloud model; no microphone, no copyrighted audio.
...
Qwen3-TTS (the narration in this video), Alibaba Cloud OSS, FastAPI, Python 3.12, Docker.
